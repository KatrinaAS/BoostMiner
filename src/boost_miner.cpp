...
Bitcoin::satoshi calculate_fee(Bitcoin::prevout p, bytes pay_script,
                               double fee_rate) {
  ...
  data::uint32 estimated_tx_size =
      ...
      + Boost::input_script::expected_size(
            output_script.Type, output_script.UseGeneralPurposeBits) +
      4                   // sequence number
  ...
  if (fee > spent)
    throw std::string{"Cannot pay tx fee with boost output"};
...
work::proof cpu_solve(const work::puzzle &p, const work::solution &initial) {
  ...
  std::copy(initial.Share.ExtraNonce2.begin(), initial.Share.ExtraNonce2.end(),
            extra_nonce_2.begin());
  ...
  while (true) {
    ...
    if (hash < best) {
      ...
      logger::log("besthash",
                  json{{"hash", best}, {"total", uint64(total_hashes)}});
    ...
    if (pr.Solution.Share.Nonce == 0) {
      extra_nonce_2++;
      std::copy(extra_nonce_2.begin(), extra_nonce_2.end(),
                pr.Solution.Share.ExtraNonce2.begin());
    }
  }
...
Boost::output_script read_output_script(int arg_count, char **arg_values) {
  ...
  if (!content.valid())
    throw(string{"could not read content: "} + content_hash_hex);
  ...
  if (!target.valid())
    throw(std::string{"could not read difficulty: "} + difficulty_input);

  // Tag/topic does not need to be anything.
  ptr<bytes> topic = encoding::hex::read(string{arg_values[2]});
  if (topic == nullptr || topic->size() > 20)
    throw(std::string{"could not read topic: "} + string{arg_values[2]});
  ...
  if (additional_data == nullptr)
    throw(std::string{"could not read additional_data: "} +
          string{arg_values[3]});
  ...
  if (arg_count == 4) {
    output_script = Boost::output_script::bounty(
        category, content, target, *topic, user_nonce, *additional_data,
        use_general_purpose_bits);
    logger::log("job.create",
                json{{"target", target},
                     {"difficulty", diff},
    ...
    if (!miner_address.valid())
      throw(std::string{"could not read miner address: "} +
            string{arg_values[4]});

    output_script = Boost::output_script::contract(
        category, content, target, *topic, user_nonce, *additional_data,
        miner_address.Digest, use_general_purpose_bits);
...
int command_spend(int arg_count, char **arg_values) {
  if (arg_count < 4 || arg_count > 5)
    throw std::string{"invalid number of arguments; should be 4 or 5"};
...
json solution_to_json(work::solution x) {
  ...
  if (x.Share.Bits)
    share["bits"] = data::encoding::hex::write(*x.Share.Bits);

  return json{{"share", share},
...
Bitcoin::transaction mine(
    ...
    // the address you want the bitcoins to go to once you have redeemed the
    // boost output. this is not the same as 'miner address'. This is just an
    // address in your normal wallet and should not be the address that goes
    // along with the key above.
    const Bitcoin::address &address) {
  using namespace Bitcoin;

  // Is this a boost output?
  Boost::output_script output_script{prev.script()};
  if (!output_script.valid())
    throw std::string{"Not a valid Boost output script"};

  // If this is a contract script, we need to check that the key we have been
  // given corresponds to the miner address in the script.
  if (output_script.Type == Boost::contract &&
      output_script.MinerAddress != private_key.address().Digest)
    throw "Incorrect key provided to mine this output.";

  // is the difficulty too high?
  if (output_script.Target.difficulty() > 1.01) {
    // std::cout << "warning: difficulty may be too high for CPU mining." <<
    // std::endl;
  }
  ...
  work::solution initial{timestamp::now(), 0, bytes_view(extra_nonce_2),
                         extra_nonce_1};

  if (output_script.UseGeneralPurposeBits)
    initial.Share.Bits = random_uint32(generator);
  ...
  Bitcoin::satoshi fee{
      calculate_fee(prev, pay_script, wallet::default_fee_rate)};
  ...
  Bitcoin::signature signature = private_key.sign(
      sighash::document{prev.value(), // output being redeemed
                        prev.script(),
  ...
  Boost::input_script input_script = Boost::input_script(
      signature, private_key.to_public(), proof.Solution, output_script.Type,
      output_script.UseGeneralPurposeBits);

  data::bytes redeemhex = input_script.write();

  std::string redeemhexstring = data::encoding::hex::write(redeemhex);

  logger::log("job.complete.redeemscript",
              json{{"solution", solution_to_json(proof.Solution)},
...
int command_redeem(int arg_count, char **arg_values) {
  if (arg_count != 6)
    throw std::string{"invalid number of arguments; should be 6"};
  ...
  if (script == nullptr)
    throw std::string{"could not read script"};
  ...
  if (!txid.valid())
    throw std::string{"could not read txid"};
  ...
  if (!address.valid())
    throw std::string{"could not read address"};

  Bitcoin::secret key{arg_wif};
  if (!key.valid())
    throw std::string{"could not read secret key"};

  logger::log("job.mine", json{{"script", arg_values[0]},
                               {"value", arg_values[1]},
                               {"txid", arg_values[2]},
                               {"vout", arg_values[3]},
  ...
  Bitcoin::transaction tx =
      mine(Bitcoin::prevout{Bitcoin::outpoint{txid, index},
                            Bitcoin::output{Bitcoin::satoshi{value}, *script}},
           key, address);

  std::string txhex = data::encoding::hex::write(bytes(tx));

  logger::log("job.complete.transaction", json{{"txhex", txhex}});
...
int command_boost(int arg_count, char **arg_values) {
  ...
  if (arg_count < 4 || arg_count > 7)
    throw std::string{
        "invalid number of arguments; should at least 4 and at most 7"};
  ...
  if (!content.valid())
    throw(string{"could not read content: "} + arg_content);
  ...
  if (arg_count > 4) {
    ...
    if (topic_read == nullptr || topic_read->size() > 20)
      throw(std::string{"could not read topic: "} + string{arg_values[4]});
    topic = *topic_read;
  }
  ...
  if (arg_count > 6) {
    ...
    if (boost_type == "contract")
      type = Boost::contract;
    else if (boost_type != "bounty")
      throw(std::string{"could not boost type: "} + string{arg_values[6]});
  }

  if (type == Boost::contract)
    throw std::string{"We do not do boost contract yet"};

  Boost::output_script boost_output_script = Boost::output_script::bounty(
      0, content, target, topic, random_uint32(get_random_engine()),
  ...
  if (boost_output_index == spend.Transaction.Outputs.size())
    throw "could not find boost output index";
  ...
  Bitcoin::transaction redeem =
      mine(Bitcoin::prevout{Bitcoin::outpoint{spend.Transaction.id(),
                                              boost_output_index},
                            bitcoin_output},
           miner_key, spend_key.address());

  broadcast(redeem);

  w = w.add(p2pkh_prevout{redeem.id(), 0, redeem.Outputs[0].Value, spend_key});
...
int help() {
  ...
  std::cout
      << "input should be \"function\" \"args\"... where function is "
         ...
         "\n\taddress    -- OPTIONAL. If provided, a boost contract output "
         "will be created. Otherwise it will be boost bounty."
         ...
         "\n\twif        -- private key that will be used to redeem this "
         "output."
         ...
         "\n\ttype       -- OPTIONAL: may be either 'bounty' or 'contract'. "
         "Default is 'bounty'"
      << std::endl;
...
int main(int arg_count, char **arg_values) {
  ...
  try {
    if (function == "spend")
      return command_spend(arg_count - 2, arg_values + 2);
    if (function == "redeem")
      return command_redeem(arg_count - 2, arg_values + 2);
    if (function == "boost")
      return command_boost(arg_count - 2, arg_values + 2);
    if (function == "help")
      return help();
    help();
  } catch (std::string x) {
