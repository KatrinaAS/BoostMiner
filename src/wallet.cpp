...
p2pkh_prevout read_prevout(const json &j) {
  ...
  return p2pkh_prevout{
      ...
      Gigamonkey::digest256{string(j["txid"])}, data::uint32(j["index"]),
...
wallet read_json(std::istream &i) {
  ...
  if (j.size() != 3 || !j.contains("prevouts") || !j.contains("master") ||
      !j.contains("index"))
  ...
  for (const json p : pp)
    prevouts <<= read_prevout(p);

  return wallet{prevouts, hd::bip32::secret{string(j["master"])},
                data::uint32(j["index"])};
}

Bitcoin::satoshi wallet::value() const {
  return data::fold(
      [](Bitcoin::satoshi x, const p2pkh_prevout &p) -> Bitcoin::satoshi {
        return x + p.Value;
      },
...
wallet::spent wallet::spend(Bitcoin::output to,
                            double satoshis_per_byte) const {
  wallet w = *this;
  ...
  bytes change_script =
      Gigamonkey::pay_to_address::script(new_secret.address().Digest);

  // we create a transaction design without inputs and with an empty change
  // output. we will figure out what these parameters are.
  Gigamonkey::transaction_design tx{
      1, {}, {to, Bitcoin::output{0, change_script}}, 0};
  ...
  while (true) {
    ...
    // we have enough funds to cover the amount sent with fee without leaving a
    // dust input.
    if (tx.spent() - tx.sent() - fee > dust)
      break;

    if (data::empty(w.Prevouts))
      throw std::string{"insufficient funds"};

    auto p = w.Prevouts.first();
    w.Prevouts = w.Prevouts.rest();

    tx.Inputs <<= transaction_design::input(
        Bitcoin::prevout(p), redeem_script_size(p.Key.Compressed));
    signing_keys <<= p.Key;
  }

  // randomize change index.
  uint32 change_index =
      std::uniform_int_distribution<data::uint32>(0, 1)(get_random_engine());

  Bitcoin::satoshi change_value = tx.spent() - tx.sent() - fee;
  Bitcoin::output change{change_value, change_script};

  // replace outputs with a randomized list containing the correct change
  // amount.
  tx.Outputs = change_index == 0 ? list<Bitcoin::output>{change, to}
                                 : list<Bitcoin::output>{to, change};

  Bitcoin::incomplete::transaction incomplete(tx);

  list<Bitcoin::sighash::document> documents = tx.documents();

  Bitcoin::transaction complete = incomplete.complete(data::map_thread(
      [](const Bitcoin::secret &k,
         const Bitcoin::sighash::document &doc) -> bytes {
  ...
  list<Bitcoin::result> results = data::map_thread(
      [&index, &incomplete](const transaction_design::input &inp,
                            const Bitcoin::input &inb) -> Bitcoin::result {
        return Bitcoin::evaluate(inb.Script, inp.Prevout.script(),
                                 Bitcoin::redemption_document{
                                     inp.Prevout.value(), incomplete, index++});
      },
      tx.Inputs, complete.Inputs);

  return spent{w.add(p2pkh_prevout{complete.id(), change_index, change_value,
                                   new_secret}),
               complete};
}
