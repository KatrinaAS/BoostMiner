...
int command_generate(int arg_count, char **arg_values) {
  if (arg_count != 1)
    throw "invalid number of arguments; one expected.";

  std::string filename{arg_values[0]};

  std::cout << "Type random characters and we will generate a wallet for you. "
...
int command_import(int arg_count, char **arg_values) {
  if (arg_count != 5)
    throw std::string{"invalid number of arguments; five expected."};
  ...
  if (!key.valid())
    throw "could not read secret key";
...
int command_send(int arg_count, char **arg_values) {
  if (arg_count != 3)
    throw std::string{"invalid number of arguments; three expected."};
...
int help() {
  ...
  std::cout << "input should be \"function\" \"args\"... where function is "
               ...
               "\n\tfilename   -- wallet file name. "
               "\n\ttxid       -- txid of the tx that contains the output to "
               ...
               "\n\taddress    -- address to send them to."
            << std::endl;
...
int main(int arg_count, char **arg_values) {
  ...
  try {
    if (function == "generate")
      return command_generate(arg_count - 2, arg_values + 2);
    if (function == "import")
      return command_import(arg_count - 2, arg_values + 2);
    if (function == "receive")
      return command_receive(arg_count - 2, arg_values + 2);
    if (function == "send")
      return command_send(arg_count - 2, arg_values + 2);
    if (function == "help")
      return help();
    help();
  } catch (std::string x) {
